var updateView = async(button) => {
  switch(button.dataset.querytype){
    ...
    case 'by_instructor':
      ...
      api = `http://localhost:${port}/api/by_instructor/${queryName}`;
      break;
    case 'by_code':
      let queryCode = document.querySelector("#codeQuery").value;
      api = `http://localhost:${port}/api/by_course_code/${queryCode}`;
      break;
    case 'by_level':
      console.log(button.dataset.querytype);
      ...
      api = `http://localhost:${port}/api/by_level/${queryLevel}`;
      break;
    case 'dual_query':
      ...
      let dual_queryLevel;
        for (const dual_choice of dual_choices) {
            if (dual_choice.checked) {
              ...
              api = `http://localhost:${port}/api/combined_query/${dual_queryName}/${dual_queryLevel}`;
        break;
